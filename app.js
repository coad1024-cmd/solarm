 ...
 * Minimalist Client-Side Interactive Logic
 */

// Data: The 24 Solana/Agave Native ARM64 Binaries
const SOLANA_ARM64_BINARIES = [
  { name: "solana", category: "Core CLI", size: "36.8 MB", status: "Verified", desc: "Main command-line interface for Solana blockchain interaction and account management" },
  ...
  { name: "solana-test-validator", category: "Validator", size: "45.1 MB", status: "Verified", desc: "Local testnet validator node for fast smart contract development and RPC emulation" },
  ...
  { name: "cargo-test-sbf", category: "Toolchain", size: "19.2 MB", status: "Verified", desc: "Runs unit and integration tests for SBF smart contracts in local VM environment" },
  { name: "cargo-build-bpf", category: "Toolchain", size: "18.9 MB", status: "Verified", desc: "BPF program compilation harness wrapper for backward compatibility" },
  { name: "cargo-test-bpf", category: "Toolchain", size: "19.2 MB", status: "Verified", desc: "BPF program test execution wrapper" },
...
>>> Ultra-fast in-memory SVM test completed in 150 milliseconds!`
  ...
  docker: {
    ...
    output: `Unable to find image \x27ghcr.io/solarm-org/solana-arm64:v1.18.26\x27 locally
  ...
  function renderTable(filterText = "") {
    ...
    tableBody.innerHTML = filtered.map(item => `
      <tr>
        ...
        <td><span class="bin-badge">${item.category}</span></td>
        <td><span class="bin-badge" style="color: #111111; font-weight: 600;">${item.status}</span></td>
        <td style="font-family: var(--font-mono); font-size: 0.82rem;">${item.size}</td>
        <td>${item.desc}</td>
        <td>
